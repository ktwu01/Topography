...
df.rename(columns={'LTA_P_mmpa': 'Precipitation', 'LTA_PET_mm': 'Potential Evapotranspiration', 'Recharge_mmpa': 'Recharge',
                   'Long': 'lon', 'Lat': 'lat'}, inplace=True)
...
#                domains="aridity_class", palette=palette, alpha=0.1, s=10)
...
g = sns.FacetGrid(df, col="dummy", col_wrap=4)
#g.map_dataframe(plotting_fcts.plot_coloured_scatter_random, x_name, y_name,
#                domains="aridity_class", alpha=1, s=10)
g.map_dataframe(plt.scatter, x_name, y_name,  alpha=1, s=10, lw = 0, color='lightgrey')
